...
export default function SettingsManagement() {
  ...
  const validateSettings = (): boolean => {
    ...
    if (settings.aboutUsUrl && !validateUrl(settings.aboutUsUrl)) {
      newErrors.aboutUsUrl = t('admin.settings.validation.invalidUrl');
    }
    if (settings.termsUrl && !validateUrl(settings.termsUrl)) {
      newErrors.termsUrl = t('admin.settings.validation.invalidUrl');
    }
    if (settings.privacyUrl && !validateUrl(settings.privacyUrl)) {
      newErrors.privacyUrl = t('admin.settings.validation.invalidUrl');
    }
    ...
    <Box>
      ...
      <Paper sx={{ p: 3 }}>
        <Grid container spacing={3}>
          ...
          <Grid size={{ xs: 12, md: 4 }}>
            <TextField
              label={t('admin.settings.aboutUs')}
              value={settings.aboutUsUrl}
              onChange={(e) => handleChange('aboutUsUrl', e.target.value)}
              fullWidth
              placeholder="/about"
              error={!!errors.aboutUsUrl}
              helperText={errors.aboutUsUrl || t('admin.settings.urlHint')}
            />
          </Grid>

          <Grid size={{ xs: 12, md: 4 }}>
            <TextField
              label={t('admin.settings.terms')}
              value={settings.termsUrl}
              onChange={(e) => handleChange('termsUrl', e.target.value)}
              fullWidth
              placeholder="/terms"
              error={!!errors.termsUrl}
              helperText={errors.termsUrl || t('admin.settings.urlHint')}
            />
          </Grid>

          <Grid size={{ xs: 12, md: 4 }}>
            <TextField
              label={t('admin.settings.privacy')}
              value={settings.privacyUrl}
              onChange={(e) => handleChange('privacyUrl', e.target.value)}
              fullWidth
              placeholder="/privacy"
              error={!!errors.privacyUrl}
              helperText={errors.privacyUrl || t('admin.settings.urlHint')}
            />
          </Grid>
        </Grid>
      </Paper>
