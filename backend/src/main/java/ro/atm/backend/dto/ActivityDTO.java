    ...
    public static ActivityDTO fromEntity(Activity activity) {
        return ActivityDTO.builder()
                ...
                .imageUrls(activity.getMediaList().stream()
                        .filter(m -> m.getMediaType() == ro.atm.backend.entity.Media.MediaType.IMAGE)
                        .map(ro.atm.backend.entity.Media::getUrl)
                        .toList())
                .videoUrls(activity.getMediaList().stream()
                        .filter(m -> m.getMediaType() == ro.atm.backend.entity.Media.MediaType.VIDEO)
                        .map(ro.atm.backend.entity.Media::getUrl)
                        .toList())
                .build();
    }
